...
const rasa_server_url = "http://localhost:8000/predict";
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const BotResponse = `<img class="botAvatar" src="./static/img/sara_avatar.png"/><p class="botMsg">${response['text']}</p><div class="clearfix"></div>`;
...
function customActionTrigger() {
  $.ajax({
    url: "http://localhost:5055/Bank/abc",
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({
